(function () {
  ...
  function decorateText(text) {
    ...
    while ((m = blockRe.exec(text)) !== null) {
      ...
      parts.push({ block: true, code: m[1] });
      last = m.index + m[0].length;
    }
    parts.push(text.slice(last));
    return parts.map(function (p) {
      if (typeof p === 'object') return '<pre>' + esc(p.code) + '</pre>';
...
async function chatRequest(messages) {
  ...
  async function onSend(rawText) {
    ...
    try {
      ...
      current.items.push({ role: 'assistant', content: content || '(respons kosong)' });
      addAssistantMessage(content || '(respons kosong)', usage || undefined);
